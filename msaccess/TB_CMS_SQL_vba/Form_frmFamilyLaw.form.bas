...
Private Sub Date_of_Separation_BeforeUpdate(Cancel As Integer)
    If Not IsNull(Me.Date_of_Separation) Then
        If IsNull(Me.Date_of_Marriage) Then
            MsgBox "Kindly enter marriage date first.", vbInformation, "TB CMS: Invalid Input"
            Cancel = True
        Else
            If Me.Date_of_Separation < Me.Date_of_Marriage Then
                MsgBox "Date of separation can't be earlier than date of marriage.", vbInformation, "TB CMS: Invalid Input"
                Cancel = True
            Else
...
'    Response = 0
'    If MsgBox("Are you sure you want to delete this record?", vbYesNo + vbQuestion) = vbNo Then Cancel = True
'End Sub
