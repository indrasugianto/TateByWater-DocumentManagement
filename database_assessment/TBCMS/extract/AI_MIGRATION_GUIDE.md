...
- **Extracted (UTC):** 2026-08-27T13:21:22.593451+00:00
- **Run success:** False
...
> ⚠️ **Partial extraction — do not assume full coverage.** The following objects were skipped and must be inspected manually:
> - `reportErrors`: {'report': 'rpt_Billing_Closing', 'error': 'skipped via --skip-report'}
...
- **Entry point:** `frmLogin` (database-properties).
...
- **VBA red flags:** `createObject`×23, `runSql`×46, `setWarnings`×4, `outputTo`×8, `fileSystem`×7, `eval`×1, `transferSpreadsheet`×1, `transferText`×1 — see `vba/index.json` for the exact procedures.
...
7. **Sequence the work** — group into phases (data layer → API → screens → reports), ordered by dependency (entry point `frmLogin` and its immediate navigation first).
