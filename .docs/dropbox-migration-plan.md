...
**What changed this session (2026-06-29) — Phase 4e: Explorer-first folder open (G24):**

`DocumentManagement.OpenDocumentFolder` now opens the case folder in **Windows Explorer** (the firm's preferred, more familiar view) when the Dropbox desktop client is present and the folder has synced, falling back to the Dropbox web URL otherwise. New private helper `OpenFolderInExplorerOrWeb` revives the previously-dormant Phase 4b machinery (`DropboxService.DropboxPathToLocalPath` + `ResolveLocalSyncedRoot`). **Default-safe:** on a machine with no Dropbox client the behavior is identical to the prior web-only open, so it can ship before any client rollout. No SQL/bridge/schema change; create-on-demand and the `ALLOW_DROPBOX_WRITES` kill-switch are untouched. Implements the **G24** decision (open in Explorer when a synced local mount exists). NOTE: with this edit the working tree is no longer clean — `DocumentManagement.bas` change is pending commit. Open question for IT/leadership before this lights up: whether the Dropbox desktop client (online-only) can be deployed on the locked-down workstations, and whether users may write directly in Explorer (outside the bridge audit trail) or the `/Company` team folder should be permissioned read-only for staff.

**What changed this session (2026-06-03) — OAuth hardened for locked-down, non-admin workstations:**

A real tester (`sugianto`) hit *"Dropbox session could not be initialized at step 'LoadTokens'"* on a locked-down workstation. Root cause was two-layered; both are fixed in `DropboxService.bas`:

- **The stored DPAPI token was unreadable (expected).** The `tblDropboxTokens` blob had been encrypted under a *different Windows profile*, so `LoadTokens` failed with `0x80090005 NTE_BAD_DATA` and correctly fell through to re-auth. Folder-open kept working throughout because it uses the Dropbox **desktop-sync mount**, not an API token — a misleading "it still works" signal worth remembering (see G18 / new **G27**).
- **Re-auth then crashed** because the OAuth flow shelled a PowerShell `HttpListener` on `:8765`, which a no-admin, security-locked workstation cannot do (`powershell.exe` blocked by AppLocker/SmartScreen; `HttpListener.Start` needs admin/URL-ACL). `Shell` raised runtime **Err 5** *before* the browser ever opened.

Fixes (committed to `DropboxService.bas`):
1. **`USE_LOCAL_LISTENER = False` is now the default.** OAuth uses the manual-paste InputBox flow (no PowerShell, no port bind). Requires `http://localhost` registered in the Dropbox App Console (it is).
2. **`OpenBrowser` rewritten to `Shell "explorer.exe ""<url>"""`** — launches the default browser (Edge) via the running shell process: not AppLocker/SmartScreen-blocked, needs no admin, `&`-safe inside quotes. Replaces the `Application.FollowHyperlink` call that threw Err 5 on the long OAuth URL. The interim `ShellExecuteW` declares were removed.
3. **`StartupBootstrap` error handler** now captures `Err.Number`/`Err.Description` into locals *before* `LogLocal` runs (its trailing `Err.Clear` had been blanking the surfaced message — which is why the first error box showed a bare colon).
...
Verified before commit: `Phase4e_CreateFolderSmokeTest` returned OK (created + verified + idempotent-create; remote deleted), and the `frmClientLedger` Create-Sub-folder UI path created + opened a folder. **Re-confirmed 2026-06-04 in a live UI session on the tester's own locked-down workstation:** clicking **Create Folder** (`cmdCreateFolder` → `OpenDocumentFolder(CaseID, "General")`) prompted *"Do you want to create it now?"* → Yes → created the real folder in `/Company` via `create_folder_v2`. `ALLOW_DROPBOX_WRITES` flipped back to `False` in committed source (working tree clean).

> **Operational lesson — the re-import gotcha (2026-06-04).** The running `.accde` had a stale, pre-G27 `DocumentManagement` compiled in. It surfaced first as the *old* read-only "folder will be created automatically when the first document is saved" message (that wording was introduced in `cc78413` and removed in `c225db3` — confirm with `git log -S`), then as a *"`OpenDocumentFile()` can't be found"* compile error. Diagnosis: when a module is stale / half-imported / duplicated, it fails to compile, and a non-compiling module makes **all** its public members invisible to form callers ("can't be found" is the symptom, not the cause). **Fix = re-import _both_ `DocumentManagement.bas` and `DropboxService.bas` (they reference each other), delete any duplicate `…1` modules the importer created, then Debug → Compile.** Re-importing only one module is not enough.
...
**What changed last session (2026-05-17):**
...
- **Dropbox desktop client is no longer a deployment prerequisite** (deliberate design decision — see also `memory/feedback_no_dropbox_desktop.md`):
  - `OpenDocumentFolder` simplified to web-only: stripped the Explorer-first branch, opening the Dropbox web URL. **(Superseded 2026-06-03/04 by G27: the launch is now `Shell "explorer.exe"` instead of `Application.FollowHyperlink` — Err-5-safe on locked-down PCs — and a create-on-demand prompt was added ahead of the open. See the create-folder section above.)** Users accept the cosmetic "Unsupported path provided" banner Dropbox emits for `/work/` team-namespace deep links.
  ...
  - **Reversed 2026-08-28:** firm policy now assumes the Dropbox desktop client *is* installed on every workstation — the "no longer a deployment prerequisite" premise above no longer holds. `memory/feedback_no_dropbox_desktop.md` has been updated to record this reversal. Note Phase 4e (2026-06-29, commit `e6f66ac`) had already partly re-enabled the Explorer-first path in `OpenDocumentFolder` for other reasons (G24, "firm's preferred, more familiar view") before this policy reversal was stated explicitly — see that session's entry above. `GetScannerFolder` still returns `""` as of this note; not changed as part of this pass (comments-only update), but it's the function to revisit if scanner-folder routing is picked up as follow-up work.
...
- **Real-tenant write verification — two new smoke tests added + passed against `/Company`**:
  ...
  - **`ALLOW_DROPBOX_WRITES` is back to `False`** in the committed source. To re-run write tests: flip the constant near the top of `DropboxService.bas` (the `Public Const ALLOW_DROPBOX_WRITES` line, ~line 86), re-import the module, **Debug → Compile**, run, then flip back to `False` and re-import.

**What's done across all sessions through 2026-06-04:**

- **Phase 3a–3e** — `DropboxService.bas` (~3561 lines, sections 1–27 plus 25b `Phase4d_UploadSmokeTest` and 25c `Phase4e_CreateFolderSmokeTest`). DPAPI, kill-switch, namespace header, audit/local logging, full OAuth code-exchange flow (manual-paste by default since 2026-06-03 via `USE_LOCAL_LISTENER = False` + `explorer.exe` browser launch; PowerShell HttpListener retained in source but unusable on locked-down workstations; identity + revocation + refresh), read-only API ops (`OpenDocument`, `GetMetadata`, `ListFolder`, `GetTemporaryLink`, `CleanupTempFiles`), gated write API ops (`UploadFile`, `UploadLargeFile`, `MoveFile`, `CopyFile`, `DeleteFile`, `CreateFolder` — every entrypoint calls `GuardWritesEnabled`), startup orchestration (`StartupBootstrap` / `StartupShutdown` wired into `frmHome.Form_Open` / `Form_Unload`). Smoke tests: `Phase3a_SmokeTest`, `Phase3b_Pass1_SmokeTest` / `Pass2_UnitTest` / `Pass2_AuthFlowTest`, `Phase3c_SmokeTest` / `OpenDocumentTest`, `Phase3d_SmokeTest`, `Phase3e_SmokeTest`, `Phase4b_SmokeTest`, `Phase4d_UploadSmokeTest`, `Phase4e_CreateFolderSmokeTest` — all green on the dev machine. Authenticated as `sugianto@tatebywater.com`.
...
- **Phase 4a–4d** — `DocumentManagement.bas` (~1905 lines after the Phase4 follow-up trims + `Phase5_E2E_HappyPathTest` addition). Every document operation delegates to `DropboxService`:
  ...
  - `OpenDocumentFolder` → `GetMetadata` pre-check, then **create-on-demand** (G27): if the folder is missing, prompt *"Do you want to create it now?"* → `DropboxService.CreateFolder` (gated write) → open the Dropbox web URL via `explorer.exe`. Verified in a live UI session 2026-06-04 (see create-folder section above).
...
- `Dropbox-Migration/DropboxService.bas` — ~3561 lines, sections 1–27 (plus Section 25b `Phase4d_UploadSmokeTest` and 25c `Phase4e_CreateFolderSmokeTest`). No further changes anticipated for Phases 5–6 _besides_ smoke tests; production code is stable.
- `Dropbox-Migration/DocumentManagement.bas` — ~1953 lines, 20 public functions (all SP wrappers preserved with stable signatures + 5 workflow functions rewired — incl. `OpenDocumentFolder` create-on-demand, G27 — + 2 config functions rewired) + 1 private helper + `Phase5_E2E_HappyPathTest` at the end. All 8 form callers in the plan's Phase 4 table call the same public functions with the same signatures as before.
...
6. (Optional, write-path re-verification — requires `ALLOW_DROPBOX_WRITES = True` flip + revert):
   ...
   - `? DropboxService.Phase4e_CreateFolderSmokeTest` — create / verify / idempotent re-create / delete a throwaway folder under `/Company/__smoke_test__/` (backs the G27 create-on-demand flow).
...
## Implementation status snapshot (2026-06-04)
...
| **3** DropboxService.bas hardening | **Done** (3a–3e) | `DropboxService.bas` (~3561 lines, sections 1–27 + 25b + 25c). SQL config load, DPAPI, kill-switch, namespace header, audit/local logging; `tblDropboxTokens` upgrade + DPAPI token persistence; full OAuth code-exchange flow (manual-paste default since 2026-06-03 + `explorer.exe` launcher for locked-down workstations; PowerShell HttpListener retained but unused; identity, revocation, refresh, `EnsureValidToken`); read-only ops (`OpenDocument`, `GetMetadata`, `ListFolder`, `GetTemporaryLink`); six gated write entrypoints + chunked upload, each guarded by `GuardWritesEnabled`; `StartupBootstrap`/`StartupShutdown` wired into `frmHome`, TEST-ENVIRONMENT banner via `SetOption "Application Title"`. Smoke tests green on the dev machine. |
| **4** DocumentManagement compatibility | **Done** (4a–4d) | `DocumentManagement.bas` (~1953 lines); all 20 public functions preserved verbatim (SP wrappers) or rewired to delegate to `DropboxService`. Installer Section 8 (live on `awsql2022dev/TateByWater`) recreates the path-building SPs against `tblDropboxRootConfig`, plus the G2 `spMoveDocumentFolder` rewrite and G13 `spSaveCaseDocument` guard. Document-open round-trip validated on real cases; folder-open does create-on-demand (G27 — prompt → `CreateFolder`, verified live 2026-06-04) then opens **Explorer-first with web fallback** (Phase 4e, G24 — new `OpenFolderInExplorerOrWeb` reuses `DropboxPathToLocalPath`/`ResolveLocalSyncedRoot`); write flows guard-block cleanly with a friendly test-env MsgBox. Legacy bodies preserved for rollback. |
...
- OAuth callback capture — manual-paste InputBox is the **deployed default** (`USE_LOCAL_LISTENER = False`); the local HTTP listener on `localhost:8765` (validated in `DropboxOAuthTest.bas`, May 2026) was **retired as the default on 2026-06-03** because locked-down, non-admin workstations cannot run `powershell.exe` / bind the port (see NEXT SESSION 2026-06-03 + G27)
...
| Redirect URIs ✅ confirmed | App configuration | `http://localhost` (**deployed default** — manual-paste flow, `USE_LOCAL_LISTENER = False` since 2026-06-03) and `http://localhost:8765` (local HTTP listener — retired as default; works only on workstations where PowerShell + port binding are permitted). **Both must stay registered.** Both confirmed working in `DropboxOAuthTest.bas` (May 2026). |
...
| `RedirectUri` | NVARCHAR(500) NOT NULL | Informational only — the OAuth flow builds the redirect URI from `USE_LOCAL_LISTENER` in `DropboxService.bas`, not from this column. Seed `http://localhost` (the manual-paste flow, the deployed default since 2026-06-03). `http://localhost:8765` is the retired local-listener URI. **Both must be registered in the Dropbox App Console.** |
...
- **Document open — download-to-temp + native open**: replace `FollowHyperlink(localPath)` with `DropboxService.OpenDocument(DocumentFileName)`, which:
  ...
  3. Hands the local file path to `Shell "explorer.exe ""<path>"""` (G27, since 2026-06-03 — replaced `Application.FollowHyperlink`, which can raise Err 5 on locked-down workstations) so it opens in the native app (Word/Acrobat/Excel) — preserves today's attorney UX.
...
Create `tblDropboxConfig`, `tblDropboxRootConfig`, `tblDropboxRevocationList`, `tblDropboxAuditLog`, `tblDropboxVerificationReport` in **production** SQL Server per Phase 2 schema. Insert the single shared `tblDropboxConfig` row with `AppKey`, **rotated** `AppSecret`, `RedirectUri = http://localhost` (the manual-paste default; this column is informational — see G27). Confirm the production `.accde` build ships with `USE_LOCAL_LISTENER = False` and the `explorer.exe` `OpenBrowser` launcher (G27), and that both `http://localhost` and `http://localhost:8765` are registered in the Dropbox App Console. Insert the single `tblDropboxRootConfig` row with the confirmed namespace ID `14334595683` and the folder templates already validated in the test environment. Lock UPDATE permission on all five tables to the IT admin SQL login.
...
Picked option (a): `files/download` → `%TEMP%\TBCMS\<GUID>_<filename>` → native-app open. The launcher was migrated from `Application.FollowHyperlink` to `Shell "explorer.exe ""<path>"""` on 2026-06-03 (see G27) — FollowHyperlink can raise Err 5 on locked-down workstations; explorer.exe opens the file in its associated app via the running shell process (no admin, not policy-blocked). Phase 5 step 1 and Design Decisions ("Document open") updated. Known trade-off: edits made in the native app are not re-uploaded automatically — users must save changes back through the Save flow. This is a behavioral change from today's UNC-share flow (where edits land on the share implicitly) and must be called out in the user runbook. Browser-open via `files/get_temporary_link` remains available for non-routine link-distribution scenarios.
...
**G27. OAuth on locked-down / non-admin workstations — resolved + implemented (2026-06-03).** The firm's workstations are security-locked and users have no admin rights. The original OAuth callback path shelled `powershell.exe` to run a `System.Net.HttpListener` on `localhost:8765`; on such workstations this is impossible — `powershell.exe` is AppLocker/SmartScreen-blocked and `HttpListener.Start` requires admin/URL-ACL rights — so `Shell` raised runtime Err 5 during `AuthenticateUser`, surfacing as *"Dropbox session could not be initialized at step 'LoadTokens'"*. **Resolution** (in `DropboxService.bas`): (1) `USE_LOCAL_LISTENER = False` is the deployed default — OAuth uses the manual-paste InputBox flow, which launches no PowerShell and binds no port; (2) `OpenBrowser` launches the browser via `Shell "explorer.exe ""<url>"""` (the running shell process — not policy-blocked, no admin, `&`-safe) instead of `Application.FollowHyperlink` (which threw Err 5 on the long OAuth URL); (3) `StartupBootstrap`'s handler captures `Err` into locals before `LogLocal` clears it. **Production-build requirement:** the production `.accde` must carry `USE_LOCAL_LISTENER = False` + the `explorer.exe` launcher, and `http://localhost` must remain registered in the Dropbox App Console (see Phase 7 Step 2). **Runbook note (pairs with G18):** because DPAPI tokens are per-Windows-profile, do **not** pre-authenticate a frontend under one Windows login and hand it to another user — `LoadTokens` will fail with `0x80090005 NTE_BAD_DATA` and the user must re-authenticate once. Note that document/folder-open via the Dropbox desktop-sync mount works *without* a valid API token, so "I can open the folder" does not indicate a healthy API session. To diagnose, read the latest Error row in the local `tblDropboxLog`.
